...
import {
  getDinamikSayfa,
  updateDinamikSayfa,
  UpdateDinamikSayfaDto,
  DinamikSayfa,
} from '@/services/dinamik-sayfalar.service';

export default function DinamikSayfaDuzenlePage() {
  ...
  const [formData, setFormData] = useState<UpdateDinamikSayfaDto>({
    baslik: '',
    slug: '',
    aciklama: '',
    filtre_kriterleri: {},
  ...
  const loadSayfa = async () => {
    try {
      setDataLoading(true);
      const sayfa = await getDinamikSayfa(parseInt(sayfaId));
      ...
      setFormData({
        ...
        filtre_kriterleri: sayfa.filtre_kriterleri,
        meta_title: sayfa.meta_title || '',
        meta_description: sayfa.meta_description || '',
        meta_keywords: sayfa.meta_keywords || '',
        kapak_gorseli: sayfa.kapak_gorseli || '',
        sira: sayfa.sira,
        durum: sayfa.durum,
        ust_icerik: sayfa.ust_icerik || '',
        alt_icerik: sayfa.alt_icerik || '',
      });

      // Filtreleri doldur
      if (sayfa.filtre_kriterleri.bolgeler) {
        setBolgeler(sayfa.filtre_kriterleri.bolgeler.join(', '));
      }
      if (sayfa.filtre_kriterleri.konseptler) {
        setKonseptler(sayfa.filtre_kriterleri.konseptler.join(', '));
      }
      if (sayfa.filtre_kriterleri.yildiz) {
        setYildizlar(sayfa.filtre_kriterleri.yildiz);
      }
      if (sayfa.filtre_kriterleri.min_fiyat) {
        setMinFiyat(sayfa.filtre_kriterleri.min_fiyat.toString());
      }
      if (sayfa.filtre_kriterleri.max_fiyat) {
        setMaxFiyat(sayfa.filtre_kriterleri.max_fiyat.toString());
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await updateDinamikSayfa(parseInt(sayfaId), {
        ...formData,
        filtre_kriterleri,
      });
      toast.success('Dinamik sayfa başarıyla güncellendi');
  ...
  return (
    <div className="@container">
      <div className="mb-6">
        <Text className="text-2xl font-bold">Dinamik Sayfa Düzenle</Text>
        <Text className="text-sm text-gray-500 mt-1">
          Filtreli otel listeleme sayfasını güncelleyin
        </Text>
      </div>
