...
import {
  getOtelOdaOzellikler,
  createOtelOdaOzellik,
  updateOtelOdaOzellik,
  deleteOtelOdaOzellik,
  OtelOdaOzellik,
  CreateOtelOdaOzellikDto,
} from '@/services/otel-oda-ozellik.service';

export default function OdaOzelliklerPage() {
  const [ozellikler, setOzellikler] = useState<OtelOdaOzellik[]>([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editingId, setEditingId] = useState<number | null>(null);
  const [formData, setFormData] = useState<CreateOtelOdaOzellikDto>({
    baslik: '',
    aciklama: '',
    ikon: '',
    sira: 0,
  ...
  const loadOzellikler = async () => {
    try {
      setLoading(true);
      const data = await getOtelOdaOzellikler();
      setOzellikler(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateOtelOdaOzellik(editingId, formData);
        toast.success('Özellik güncellendi');
      } else {
        await createOtelOdaOzellik(formData);
  ...
  const handleEdit = (ozellik: OtelOdaOzellik) => {
  ...
  return (
    <div className="@container">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <Text className="text-xl font-semibold">Oda Özellikleri</Text>
          <Text className="text-sm text-gray-500">
            Otel odalarının özelliklerini yönetin
          </Text>
        </div>
        <Button
        ...
        <div className="m-auto px-8 pb-8 pt-6 w-full">
          <div className="mb-7">
            ...
            <Text className="text-sm text-gray-500 mt-1">
              Oda özelliği bilgilerini doldurun
            </Text>
          </div>
